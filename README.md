...
│
├── src/
│ ├── stt.py # Módulo de transcrição de áudio (Speech-to-Text)
...
│ ├── modeling.py # Modelagem e predição de satisfação do usuário (IA)
│ └── dashboard.py # Dashboard interativo para visualização dos resultados
│
├── notebooks/
│ └── analysis.ipynb # Análises exploratórias e gráficos complementares
│
├── results/
│ ├── reports/ # Relatórios automáticos gerados pelo pipeline
│ └── charts/ # Gráficos e visualizações exportadas
│
├── docs/
│ └── metodologia_pipeline.md # Descrição técnica e metodológica do pipeline
│
├── logs/
│ └── execution.log # Logs de execução e registro de erros
│
